...
class SecondFragmentAdapter(private val context: Context, val items: MutableList<Equipment.ItemEquipment?>) :
    ...
    inner class ViewHolder(val binding: FragmentSecondItemBinding) :
        ...
        fun bindItems(item: Equipment.ItemEquipment?) {
            ...
            when(item?.potentialOptionGrade){
                ...
                else -> binding.tvAddTvOption3.setTextColor(ContextCompat.getColor(context, R.color.Normal))
            }
            // 에디셔널 잠재능력 색상
    ...
    fun changedText(Option: String?): String {
        return when (Option) {
            "보스 몬스터 공격 시 데미지 : +18%" -> "보공18%"
            "보스 몬스터 공격 시 데미지 : +12%" -> "보공12%"
            "크리티컬 데미지 : +3%" -> "크뎀 3%"
            "크리티컬 데미지 : +1%" -> "크뎀 1%"
            "모든 스킬의 재사용 대기시간 : -1초(10초 이하는 5%감소, 5초 미만으로 감소 불가)" -> "쿨감-1초"
            "아이템 드롭률 : +5%" -> "드랍 5%"
            "메소 획득량 : +5%" -> "메획 5%"
            "캐릭터 기준 9레벨 당 STR : +2" -> "렙당STR:+2"
            "캐릭터 기준 9레벨 당 DEX : +2" -> "렙당DEX:+2"
            "캐릭터 기준 9레벨 당 INT : +2" -> "렙당INT:+2"
            "캐릭터 기준 9레벨 당 LUK : +2" -> "렙당LUK:+2"
            "캐릭터 기준 9레벨 당 STR : +1" -> "렙당STR:+1"
            "캐릭터 기준 9레벨 당 DEX : +1" -> "렙당DEX:+1"
            "캐릭터 기준 9레벨 당 INT : +1" -> "렙당INT:+1"
            "캐릭터 기준 9레벨 당 LUK : +1" -> "렙당LUK:+1"
            "공격 시 15% 확률로 85의 HP 회복" -> "기타 ♻️"
            "4초 당 16의 HP 회복" -> "기타 ♻️"
            "4초 당 24의 MP 회복" -> "기타 ♻️"
            "공격 시 10% 확률로 5레벨 중독효과 적용" -> "기타 ♻️"
            "공격 시 5% 확률로 2레벨 기절효과 적용" -> "기타 ♻️"
            "30% 확률로 받은 피해의 70%를 반사" -> "기타 ♻️"
            "공격 시 3% 확률로 32의 HP 회복" -> "기타 ♻️"
            "아이템 드롭률 : +20%" -> "드랍 20%"
            "메소 획득량 : +20%" -> "메획 20%"
            "크리티컬 데미지 : +8%" -> "크뎀 8%"
            ...
            "모든 스킬의 재사용 대기시간 : -1초(10초 이하는 5%감소, 5초 미만으로 감소 불가)" -> "쿨감-1초"
            "HP 회복 아이템 및 회복 스킬 효율 : +30%" -> "HP효율+30%"
            "4초 당 22의 MP 회복" -> "기타 ♻️"
            "공격 시 10% 확률로 6레벨 중독효과 적용" -> "기타"

            //보스관련옵션
            "보스 몬스터 공격 시 데미지 : +40%" -> "보공40%"
            "보스 몬스터 공격 시 데미지 : +35%" -> "보공35%"
            "보스 몬스터 공격 시 데미지 : +30%" -> "보공30%"
            "보스 몬스터 공격 시 데미지 : +20%" -> "보공20%"
            "몬스터 방어율 무시 : +40%" -> "방무40%"
            "몬스터 방어율 무시 : +35%" -> "방무35%"
            "몬스터 방어율 무시 : +30%" -> "방무30%%"
            "몬스터 방어율 무시 : +15%" -> "방무15%"
